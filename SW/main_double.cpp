...
int main(int argc, char* argv[])
{
    ...
    if (argc != 7) {
        ...
        std::cerr << "Usage: " << argv[0] << " order(lpf/hpf/notch),  filter freq in Hz, order, type(sine/triangle/square), test freq in Hz, test periods" << std::endl;
    ...
    {
        cout << "incorrect filter, use lpf, hpf, bpf or notch" << endl;
        return 1;
    }
    ...
    int testtype;
    if(strncmp("sine", argv[4], 3) == 0)
    {
        testtype = 0;
    }
    else if(strncmp("triangle", argv[4], 3) == 0)
    {
        testtype = 1;
    }
    else if(strncmp("square", argv[4], 3) == 0)
    {
        testtype = 2;
    }
    else
    ...
    filter.BiQuad_new(filtertype, 0, filterfreq, 192000, filterorder);
...
void showsine(int type, int freq, int periods)
{
    ...
    c_sinewave sinewave(1.0, periods);
